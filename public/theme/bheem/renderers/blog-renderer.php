 ...
 * Blog renderer
 */
class theme_bheem_core_blog_renderer  extends core_blog_renderer  {
    ...
    public function render_blog_entry(blog_entry $entry) {
        ...
        if ( strpos($actual_link, '/index.php?entryid') == false ):
            ...
            $o = $this->output->container_start($mainclass, 'b' . $entry->id);
                $o .= $this->output->container_start('single-blog-item border p-4 rounded-2 mb-4 transition-y single-blog-item-main', '' . $entry->id);
                    ...
                    $o .= $this->output->container_start('blog-content');
                        $o .="<ul class='meta'>";
                            ...
                            $o .="<li><i class='ri-calendar-2-line fs-15 text-primary'></i>";
                                $o .=''.userdate($entry->created, '%d %b', 0).'';
                            $o .="</li>";

                            $o .="<li><i class='ri-user-3-line text-primary'></i>";
                                // Post by.
                                $by = new stdClass();
                        ...
                        $short_summary = implode(' ', array_slice(str_word_count($short_summary,1), 0, 12));
            ...
            $o = $this->output->container_start($mainclass, 'b' . $entry->id);
                ...
                if (!empty($entry->renderable->comment)) {
                    ...
                    $edoo_comments = new comment($cmt);
                    $edoo_comments->set_view_permission(true);
                    $edoo_comments->set_fullwidth();

                    $o .= $edoo_comments->output(true);
                }
            $o .= $this->output->container_end();
