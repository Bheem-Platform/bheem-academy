...
require_once($CFG->dirroot . '/theme/bheem/inc/course_handler/edoo_course_handler.php');

/**
 * Course renderer
 */
class theme_bheem_core_course_renderer extends core_course_renderer {
    /**
    ...
    protected function coursecat_category(coursecat_helper $chelper, $coursecat, $depth) {
        ...
        $edoo_category_link = new moodle_url('/course/index.php', array('categoryid' => $coursecat->id));

        $edoo_cat = $coursecat->id;
        $edoo_cat_summary_unclean = $chelper->get_category_formatted_description($coursecat);
        if ($edoo_cat_summary_unclean !== null) {
            $edoo_cat_summary = preg_replace("/<img[^>]+\>/i", " ", $edoo_cat_summary_unclean);
        } else {
            $edoo_cat_summary = '';
        ...
        if ($coursecat->get_children_count() > 0) {
            $edoo_items_count .= $coursecat->get_children_count() . ' ' . get_string('categories');
        } else {
            $edoo_items_count .= count($coursecat->get_courses()) . ' ' . get_string('courses');
        }
        $edoo_cat_updated = get_string('modified') . ' ' . userdate($coursecat->timemodified, '%d %B %Y', 0);
        ...
        if (isset($src)){
            ...
            $contentimages = '<img class="img-whp" src="'.$CFG->wwwroot.'/theme/bheem/pix/category.jpg">';
        ...
        <div class="col-xl-3 col-md-6">
            <div class="d-block position-relative mb-3 cat-card">
                <div class="single-onlin-class-three border rounded-2 p-3 mb-4 transition-y">
                    <div class="courses-image">
                        <a href="'.$edoo_category_link.'" class="d-block courses-img">
                            '.$contentimages.'
                        </a>
                    </div>

                    <div class="courses-content">
                        <h3 classs="text-decoration-none text-secondary">
                            <a href="'. $edoo_category_link .'">'.$categoryname.'</a>
                        </h3>

                        <ul class="star-price list-unstyled pl-0 d-flex justify-content-between align-items-center">
                            <li class="edoo-course-date">'.$edoo_cat_updated.'</li>
                            
                            <li>
                                <span class="price">'.$edoo_items_count.'</span>
                            </li>
                        </ul>
                    </div>
                </div>
    ...
    protected function coursecat_coursebox_content(coursecat_helper $chelper, $course) {
        ...
        $edooCourseHandler = new edooCourseHandler();
        $edooCourse = $edooCourseHandler->edooGetCourseDetails($courseid);
        ...
        $bheem_context = context_course::instance($course->id);
        $numberofusers = count_enrolled_users($bheem_context);

        if (isset($PAGE->category) && !empty($PAGE->category->name)) {
        ...
        if ($course->has_course_contacts()) {
            ...
            foreach ($course->get_course_contacts() as $coursecontact) {
                ...
                $edoo_course_contacts .= '<span class="edoo_course_meta_item mr10">'.$name.'</span>';
            }
        }
        ...
        // Get Teacher Name
        $teacher = '';
        foreach($edooCourse->teachers as $teacher):
            $teacher = $teacher->name;
        endforeach;
         $contenttext = '';
         
        $edooCourseDescription = strip_tags($edooCourseHandler->edooGetCourseDescription($course->id, 99999999999999));
        $edooWords = explode(' ', $edooCourseDescription);
        $wordLimit = get_config('theme_bheem', 'cdwl') ?: 15;
        $edooCourseDescription = implode(' ', array_slice($edooWords, 0, $wordLimit));

        $contenttext .= '
            <div class="col-lg-4 col-md-6">
                <div class="single-onlin-class-three border rounded-2 p-3 mb-4 transition-y">
                    <a href="'. $edooCourse->url .'" class="d-block position-relative mb-3">
                        '.$edooCourse->edooRender->coverImage.'

                        <div class="d-flex justify-content-between align-items-center position-absolute bottom-0 start-0 end-0 p-3">
                            <span class="count bg-white text-secondary">'. $edooCourse->edooRender->updatedDate .'</span>
                            <span class="count bg-success text-secondary">';
                                if($edooCourse->course_price) {
                                    $contenttext .= '
                                    '.get_config('theme_bheem', 'site_currency') .''.$edooCourse->course_price.'';
                                }else{
                                    $contenttext .= '
                                    '.get_config('theme_bheem', 'free_course_price') .'';
                                } $contenttext .= '
                            </span>
                        </div>
                    </a>
                    <h4 class="fs-4 mb-3">
                        <a href="'. $edooCourse->url .'" class="text-decoration-none text-secondary">'.$edooCourse->fullName.'</a>
                    </h4>
                    <div class="d-flex align-items-center mb-3">
                        <span class="fs-14 ms-2"><i class="ri-user-line"></i>'.$teacher.'</span>
                    </div>
                    <div class="d-flex">
                        <span class="fw-medium text-gray2 me-2">'.get_config('theme_bheem', 'total_student') .':</span>
                        <span> '.$edooCourse->enrolments.'</span>
                    </div>
                </div>
            </div>';  
