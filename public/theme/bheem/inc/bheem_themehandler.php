...
include($CFG->dirroot . '/theme/bheem/inc/edoo_globalsearch_navbar.php');
require_once($CFG->dirroot. '/theme/bheem/inc/edoo_page_handler.php');

// Add block button in editing mode.
$addblockbutton = $OUTPUT->addblockbutton();

$edooPageHandler = new edooPageHandler();
$pageheading = $edooPageHandler->edooGetPageTitle();
...
$edooUserBodyClass = 'edoo_body_class';
$extraclasses = array(
    'edoo_no_hero',
    $user_status,
    $edooUserBodyClass
...
if($isloggedin){
    $edooProfileIconUsername = $USER->firstname . ' '. $USER->lastname;
}else{
    $edooProfileIconUsername = '';
}
...
$userProfilePage = strpos($_SERVER['REQUEST_URI'], "user/profile.php") !== false;

if ((strpos($_SERVER['REQUEST_URI'], "user/index.php") !== false || strpos($_SERVER['REQUEST_URI'], "course/edit.php") !== false || strpos($_SERVER['REQUEST_URI'], "course/completion.php") !== false || strpos($_SERVER['REQUEST_URI'], "course/admin.php") !== false || $courseSectionPage || $userProfilePage) || $userProfileFromCourseParticipants){
  $sidebar_left = false;
  $sidebar_right = false;
...
if($favicon) {
    ...
    $favicon = $CFG->wwwroot . '/theme/bheem/pix/favicon.ico';
}

if (get_config('theme_bheem', 'logo_visibility') == 1) {
 ...
 * Main Logo
 */
$logo_image_width  = preg_replace("/[^0-9]/", "", get_config('theme_bheem', 'logo_image_width'));
$logo_image_height = preg_replace("/[^0-9]/", "", get_config('theme_bheem', 'logo_image_height'));
 ...
 * Mobile Logo
 */
$mobile_logo_width  = preg_replace("/[^0-9]/", "", get_config('theme_bheem', 'mobile_logo_width'));
$mobile_logo_height = preg_replace("/[^0-9]/", "", get_config('theme_bheem', 'mobile_logo_height'));
 ...
 * Footer Logo
 */
if (get_config('theme_bheem', 'footer_logo_visibility') == 1) {
    $footer_logo_visibility = false;
} else {
    $footer_logo_visibility = true;
}
$footer_logo_width  = preg_replace("/[^0-9]/", "", get_config('theme_bheem', 'footer_logo_width'));
$footer_logo_height = preg_replace("/[^0-9]/", "", get_config('theme_bheem', 'footer_logo_height'));
...
$category_title         = get_config('theme_bheem', 'category_title');
$category_content       = get_config('theme_bheem', 'category_content');
$header_search          = get_config('theme_bheem', 'header_search');
$header_btn_url         = get_config('theme_bheem', 'header_btn_url');
$header_left_btn_text   = get_config('theme_bheem', 'header_left_btn_text');
$header_left_btn_url    = get_config('theme_bheem', 'header_left_btn_url');
$header_btn_icon        = get_config('theme_bheem', 'header_btn_icon_edoo_icon_class');
$header_left_btn_icon   = get_config('theme_bheem', 'header_left_btn_icon');

$social_target = get_config('theme_bheem', 'social_target');
if($social_target == 1) {
...
$footer_copyright      = get_config('theme_bheem', 'footer_copyright');

$banner_image = $OUTPUT->get_theme_image_banner_image(null, 100);
if($banner_image) {
    $banner_image = $OUTPUT->get_theme_image_banner_image(null, 100);
}else{
    $banner_image = false;
...
$banner_shape = $OUTPUT->get_theme_image_banner_shape(null, 100);
if($banner_shape) {
    $banner_shape = $OUTPUT->get_theme_image_banner_shape(null, 100);
}else{
    $banner_shape = false;
...
$back_to_top    = get_config('theme_bheem', 'back_to_top');
$hide_global_banner    = get_config('theme_bheem', 'hide_global_banner');
$hide_guest_access_curriculum    = get_config('theme_bheem', 'hide_guest_access_curriculum');
...
if(get_config('theme_bheem', 'footer_col_1_body')){
    $footer_column_count++;
    $footer_column_1 = true;
}
if(get_config('theme_bheem', 'footer_col_2_title') || get_config('theme_bheem', 'footer_col_2_body')){
    $footer_column_count++;
    $footer_column_2 = true;
}
if(get_config('theme_bheem', 'footer_col_3_title') || get_config('theme_bheem', 'footer_col_3_body')){
    $footer_column_count++;
    $footer_column_3 = true;
}
if(get_config('theme_bheem', 'footer_col_4_title') || get_config('theme_bheem', 'footer_col_4_body')){
    $footer_column_count++;
    $footer_column_4 = true;
}
if(get_config('theme_bheem', 'footer_col_5_title') || get_config('theme_bheem', 'footer_col_5_body')){
    $footer_column_count++;
